 ...
 test("Next/Back navigate freely and stepper allows jumping", () => {
   ...
   // Click stepper "Review" (step 4) to ensure free jump
   const reviewStep = screen.getByRole("button", { name: /^review$/i });
   fireEvent.click(reviewStep);
   // In review, consent checkbox should be present
   expect(screen.getByLabelText(/i consent to submit/i)).toBeInTheDocument();
 });
